...
/**
 * ClipboardSync relay server — v2 (adds Windows -> Android push fallback)
 *
 * No auth, no encryption, no persistence beyond process memory. Two
 * devices connect with the same `pair` id and a unique `device` id.
 * Anything one device sends gets forwarded to the OTHER device(s) in the
 * same pair via WebSocket if they're currently connected.
 *
 * Android isn't connected most of the time (it only opens a WebSocket
 * briefly per send, see the Flutter app). So when there's no live WS peer
 * to deliver to, the relay falls back to a Firebase Cloud Messaging data
 * push to that pair's registered Android device — this is what makes
 * Windows -> Android delivery work without Android needing to stay
 ...
 *
 * Registration (Android only, once per app open):
 *   POST /register  { "pairId": "...", "deviceId": "...", "fcmToken": "..." }
 */
...
async function sendFcmFallback(pairId: string, event: ClipboardEvent) {
  if (!fcmReady) return;
  const token = fcmTokensByPair.get(pairId);
  if (!token) {
...
const httpServer = createServer(async (req, res: ServerResponse) => {
  ...
  if (req.method === "POST" && url.pathname === "/register") {
    try {
      ...
      fcmTokensByPair.set(pairId, fcmToken);
...
wss.on("connection", (ws: WebSocket, req: IncomingMessage) => {
  ...
  ws.on("message", (raw) => {
    ...
    // No live WebSocket peer to deliver to — this is the normal case for
    // Windows -> Android, since Android only connects briefly per-send.
    // Fall back to a push notification instead of just dropping it.
