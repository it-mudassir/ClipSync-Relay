...
/**
 * ClipboardSync relay server — v3 (adds pull-on-open + HTTP send endpoint)
 *
 ...
 * same pair via WebSocket if they're currently connected, with an FCM push
 * fallback for Android when it isn't.
 *
...
const httpServer = createServer(async (req, res: ServerResponse) => {
  ...
  if (req.method === "GET" && url.pathname === "/latest") {
    ...
    const event = lastEventByPair.get(pairId);
    res.writeHead(200, { "Content-Type": "application/json" });
